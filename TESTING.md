...
6. Node OpenTelemetry bridge (`@devtracekit/sdk`) with span ingestion.
...
3. Start DevTraceKit with the sample app:
...
If port `4318` is already in use, run DevTraceKit on alternate ports:
...
4. `x-devtracekit-trace-id` response header is present on API responses.
...
- [ ] RBAC is enforced when auth enabled (`DEVTRACEKIT_AUTH_ENABLED=true`).
...
3. Python SDK currently sends spans over DevTraceKit HTTP ingest API (not OTLP exporter yet).
...
File: `tests/devtracekit-core.test.js`
