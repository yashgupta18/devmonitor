        ...
        try:
            with urllib.request.urlopen(
                ...
                if response.status >= 300:
                    raise RuntimeError(
                        f"DevScope ingest failed with status {response.status} at {self.ingest_url}"
                    )
        except urllib.error.HTTPError as exc:
            raise RuntimeError(
                f"DevScope ingest HTTP error {exc.code} for {self.ingest_url}. "
                "Check that DevScope is running on the same port and supports /api/ingest/otel."
            ) from exc
        except urllib.error.URLError as exc:
            raise RuntimeError(
                f"DevScope ingest connection error for {self.ingest_url}: {exc.reason}. "
                "Start DevScope or update DEVSCOPE_INGEST_URL."
            ) from exc
